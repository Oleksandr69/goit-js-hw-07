...
function createBoxes(amount) {
  ...
  for (let i = 0; i < amount; i++) {
    boxeElements += `<div style="background-color: ${getRandomHexColor()}; width: ${widthBoxe}px; height: ${heightBoxe}px;"></div>`;
    heightBoxe += 10;
    widthBoxe += 10;
  }
  boxeDiv.insertAdjacentHTML('beforeend', boxeElements);
}
...
indexValue.addEventListener('change', event => {
  let index = event.currentTarget.value.trim();
